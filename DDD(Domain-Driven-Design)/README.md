...
> 
> cf. DIP를 구현하는 기법 중 하나가 DI (Dependency Injection) 이다.
...
|도메인 서비스 (Domain Service) | 특정 엔티티에 속하지 않는 도메인 로직 제공. 도메인 로직이 여러 엔티티와 밸류를 필요로 할 경우 도메인 서비스에서 로직을 구현한다. (ex. `할인 금액 계산`  쿠폰, 회원 등급, 구매 금액 등 다양한 조건을 이용해서 구현해야우 할 경)  |

### 엔티티 (Entity) 와 밸류 (Value)
* 차이점
    * 도메인 모델의 엔티티: 
        * 데이터와 함께 도메인 기능을 함께 제공하는 객체 
...
### 애그리거트 (Aggregate)
* 도메인 모델에서 전체 구조를 이해하고 복잡한 도메인 모델을 관리하는데 도움이 되는 된다.
